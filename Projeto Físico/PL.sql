...
CREATE OR REPLACE PROCEDURE COMPRAR_APP (NOME_APP VARCHAR2, USERNM VARCHAR2) IS
	PRECO_APP NUMBER;
	VALOR NUMBER;
	DATA DATE;
...
BEGIN
	...
	SELECT C.SALDO INTO VALOR FROM CONTA C WHERE C.USERNAME = USERNM AND ROWNUM = 1;

	IF VALOR >= PRECO_APP THEN
		INSERT INTO COMPRA VALUES (USERNM, NOME_APP, PRECO_APP, DATA); --ADICIONA NA BIBLIOTECA
		UPDATE CONTA SET SALDO = (SALDO - PRECO_APP) WHERE USERNAME = USERNM; --ATUALIZA O SALDO
...
CREATE OR REPLACE PROCEDURE FECHAR_APP (USERNM VARCHAR2) IS 
	DATA DATE; 
BEGIN 
    SELECT CURRENT_DATE INTO DATA FROM DUAL; 
    UPDATE HISTORICO_EXEC SET FIM = DATA WHERE USERNM = USERNAME_LOGADO AND FIM IS NULL;  
    UPDATE CONTA SET EM_USO = 0 WHERE USERNAME = USERNM; 
END;
...
CREATE OR REPLACE PROCEDURE COMPRAR_CREDITO (USERNM VARCHAR2, VALOR NUMBER) IS

BEGIN
    UPDATE CONTA SET SALDO = VALOR WHERE USERNAME = USERNM; 
	DBMS_OUTPUT.PUT_LINE('CRÉDITOS ADICIONADOS! DINHEIRO NA MÃO, CALCINHA NO CHÃO.');
END;

-- VINCULAR REDE SOCIAL --

CREATE OR REPLACE PROCEDURE VINCULAR_REDE (USERNM VARCHAR2, USERNM_REDE VARCHAR2) IS

BEGIN
    UPDATE CONTA SET USERNAME_REDE = USERNM_REDE WHERE USERNAME = USERNM; 
