...
CREATE OR REPLACE PROCEDURE GANHAR_CONQUISTA (NOME_APP2 VARCHAR2, NOME_CONQUISTA VARCHAR2, COD_USER VARCHAR2, USERNAME_LOG VARCHAR2, INICIO DATE) IS 
	DATA DATE; 
	CNT NUMBER;
BEGIN 
    SELECT CURRENT_DATE INTO DATA FROM DUAL;
	SELECT COUNT(*) INTO CNT FROM HISTORICO_EXEC WHERE (FIM IS NOT NULL) AND (NOME_APP = NOME_APP2) AND (USERNAME_LOGADO = USERNAME_LOG);

	IF CNT > 0 THEN
		INSERT INTO GANHAR_CONQ VALUES(NOME_APP2, NOME_CONQUISTA, COD_USER, USERNAME_LOG, INICIO, DATA);
