...
CREATE OR REPLACE PROCEDURE GANHAR_CONQUISTA (NOME_APP2 VARCHAR2, NOME_CONQUISTA2 VARCHAR2, USERNAME2 VARCHAR2) IS 
    DATA DATE; 
    CNT NUMBER;
    DATA_INICIO DATE;
BEGIN
    SELECT COUNT(*) INTO CNT FROM GANHAR_CONQ WHERE (NOME_APP = NOME_APP2) AND (NOME_CONQUISTA = NOME_CONQUISTA2) AND (USERNAME = USERNAME2); 
    
    IF CNT > 0 THEN
        DBMS_OUTPUT.PUT_LINE('VOCÊ JÁ TEM ESSA CONQUISTA!');
    ELSE
        SELECT CURRENT_DATE INTO DATA FROM DUAL;
        SELECT COUNT(*) INTO CNT FROM HISTORICO_EXEC WHERE (FIM IS NULL) AND (NOME_APP = NOME_APP2) AND (USERNAME = USERNAME2);
        
        IF CNT > 0 THEN
            INSERT INTO GANHAR_CONQ VALUES(NOME_APP2, NOME_CONQUISTA2, USERNAME2, DATA);
