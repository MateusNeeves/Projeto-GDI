...
CREATE OR REPLACE PROCEDURE COMPRAR_APP (NOME_APP VARCHAR, USERNAME VARCHAR) IS
	PRECO_APP NUMBER;
	SALDO NUMBER;
	DATA DATE;
...
BEGIN
	...
	SELECT C.SALDO INTO SALDO FROM CONTA C WHERE C.USERNAME = USERNAME AND ROWNUM = 1;

	IF SALDO >= PRECO_APP THEN
		INSERT INTO COMPRA VALUES (USERNAME, NOME_APP, PRECO_APP, DATA); --ADICIONA NA BIBLIOTECA
		UPDATE CONTA C SET C.SALDO = (C.SALDO - PRECO_APP) WHERE C.USERNAME = USERNAME; --ATUALIZA O SALDO
...
CREATE OR REPLACE TYPE myarray IS VARRAY(10) OF VARCHAR(20);

CREATE OR REPLACE PROCEDURE publicar_app (NOME VARCHAR, CNPJ VARCHAR, CATEGORIAS myarray, PRECO NUMBER, TIPO VARCHAR) IS
	DATA DATE;
	CNT NUMBER;
...
CREATE OR REPLACE PROCEDURE FECHAR_APP (USERNAME VARCHAR) IS 
	DATA DATE; 
BEGIN 
    SELECT CURRENT_DATE INTO DATA FROM DUAL; 
...
CREATE OR REPLACE PROCEDURE CADASTRAR_EMPRESA (CNPJ VARCHAR, NOME VARCHAR) IS
...
CREATE OR REPLACE PROCEDURE CADASTRAR_CONTA (USERNAME VARCHAR, LOC_PAIS VARCHAR, LOC_ESTADO VARCHAR, LOC_CIDADE VARCHAR) IS
...
CREATE OR REPLACE PROCEDURE COMPRAR_CREDITO (USERNAME VARCHAR, VALOR NUMBER) IS
...
CREATE OR REPLACE PROCEDURE VINCULAR_REDE (USERNAME VARCHAR, USERNAME_REDE VARCHAR) IS
...
CREATE OR REPLACE PROCEDURE FAZER_AMIZADE (USERNAME1 VARCHAR, USERNAME2 VARCHAR) IS
	DATA DATE;
	CNT NUMBER;
BEGIN
	SELECT CURRENT_DATE INTO DATA FROM DUAL;
...
CREATE OR REPLACE FUNCTION HORAS_JOGADAS (USERNAME VARCHAR, NOME VARCHAR) RETURN NUMBER IS
    SOMA NUMBER := 0;
	DIFF NUMBER;
...
CREATE OR REPLACE FUNCTION RECEITA (CNPJ VARCHAR) RETURN NUMBER IS
    SOMA NUMBER := 0;
	VALOR NUMBER;
