...
CREATE OR REPLACE PROCEDURE CADASTRAR_CONQ (NOME_JOGO VARCHAR2, NOME_CONQUISTA VARCHAR2, DESCRICAO VARCHAR2, RARIDADE VARCHAR2) IS 
    CNT NUMBER;
BEGIN
    SELECT COUNT(*) INTO CNT FROM APLICATIVO WHERE NOME = NOME_JOGO AND TIPO = 'JOGO';
    IF CNT > 0 THEN
        INSERT INTO CONQUISTA VALUES(NOME_JOGO, NOME_CONQUISTA, DESCRICAO, RARIDADE);
        DBMS_OUTPUT.PUT_LINE('CONQUISTA CADASTRADA. VAI LÁ DESBLOQUEAR ELA!');
    ELSE
        DBMS_OUTPUT.PUT_LINE('TÁ LOUCO? ESSE JOGO NÃO EXISTE!');
    END IF;
END;
...
BEGIN
    ...
    IF USO = 0 THEN
        SELECT COUNT(*) INTO CNT FROM COMPRA WHERE USERNAME = USERNAME_LOG AND NOME = NOME_APP;
        IF (CNT > 0) THEN
...
CREATE OR REPLACE PROCEDURE GANHAR_CONQUISTA (NOME_JOGO2 VARCHAR2, NOME_CONQUISTA2 VARCHAR2, USERNAME2 VARCHAR2) IS 
    DATA DATE; 
    CNT NUMBER;
    DATA_INICIO DATE;
BEGIN
    SELECT COUNT(*) INTO CNT FROM HISTORICO_EXEC WHERE (FIM IS NULL) AND (NOME_APP = NOME_JOGO2) AND (USERNAME = USERNAME2);

    IF CNT = 0 THEN
        DBMS_OUTPUT.PUT_LINE('APP TÁ FECHADO. NÃO GANHOU A CONQUISTA');
    ELSE
        SELECT COUNT(*) INTO CNT FROM GANHAR_CONQ WHERE (NOME_JOGO = NOME_JOGO2) AND (NOME_CONQUISTA = NOME_CONQUISTA2) AND (USERNAME = USERNAME2);
        IF CNT > 0 THEN
            DBMS_OUTPUT.PUT_LINE('VOCÊ JÁ TEM ESSA CONQUISTA!');
        ELSE
            SELECT CURRENT_DATE INTO DATA FROM DUAL;
            INSERT INTO GANHAR_CONQ VALUES(NOME_JOGO2, NOME_CONQUISTA2, USERNAME2, DATA);
            DBMS_OUTPUT.PUT_LINE('BOOOOOAAAA. CONQUISTA "' || NOME_CONQUISTA2 || '" DESBLOQUEADA!!');
        END IF;
    END IF;
END;
...
BEGIN
    SELECT CURRENT_DATE INTO DATA FROM DUAL;
    SELECT COUNT(*) INTO CNT FROM HISTORICO_EXEC WHERE USERNAME = USERNAME2 AND FIM IS NULL;
    
    IF CNT > 0 THEN
        UPDATE HISTORICO_EXEC SET FIM = DATA WHERE USERNAME2 = USERNAME AND FIM IS NULL;
...
CREATE OR REPLACE FUNCTION HORAS_JOGADAS (USERNAME2 VARCHAR2, NOME_APP2 VARCHAR2) RETURN NUMBER IS
    SOMA NUMBER := 0;
    DIFF NUMBER;
    CURSOR CUR IS (SELECT 24 * (FIM - INICIO) FROM HISTORICO_EXEC WHERE USERNAME = USERNAME2 AND NOME_APP = NOME_APP2);
BEGIN
    OPEN CUR;
    FETCH CUR INTO DIFF;
