...
CREATE OR REPLACE PROCEDURE COMPRAR_APP (COD_APP VARCHAR, USERNAME VARCHAR) IS
PRECO_APP NUMBER;
SALDO NUMBER;
DATA DATE;
...
BEGIN
    ...
    SELECT PRECO_ATUAL INTO PRECO_APP 
    FROM APLICATIVO A 
    WHERE A.CODIGO = COD_APP;
    ...
    IF SALDO >= PRECO_APP THEN
        ...
        UPDATE CONTA C              --ATUALIZA O SALDO
        SET SALDO = (SALDO - PRECO_APP) 
        WHERE C.USERNAME = USERNAME;
